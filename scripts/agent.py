...
class Agent:
    def __init__(self, input_size, output_size, double_dqn=True):
    ...
    def load(self):
        if os.path.exists("../backup/backup.q"):
            self.q_network.load_state_dict(torch.load("../backup/backup.q"))
        if os.path.exists("../backup/backup.target"):
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(self.v_stack_impr([e.state for e in experiences if e is not None])) \
            .float().to(device)
        actions = torch.from_numpy(self.v_stack_impr([e.action for e in experiences if e is not None])) \
            .long().to(device)
        rewards = torch.from_numpy(self.v_stack_impr([e.reward for e in experiences if e is not None])) \
            .float().to(device)
        next_states = torch.from_numpy(self.v_stack_impr([e.next_state for e in experiences if e is not None])) \
            .float().to(device)
        dones = torch.from_numpy(self.v_stack_impr([e.done for e in experiences if e is not None]).astype(np.uint8)) \
            .float().to(device)
